...
(function($){$.fn.viewbox = function(options){
	...
	function show($e){
		...
		var href = $e.attr('href')
			,caption = options.setTitle ? $e.data('viewbox-title') || $e.data('viewboxtitle') || $e.attr('title') : '';
		if(!href){
			$current = $e;
			showPopup($e,caption);
	...
	function addSvgButton(name){
		...
		$e.appendTo($container.find(".viewbox-body"))
	...
	function showImage(href,caption){
		...
		var $body = get('body')
			...
			,timerId = window.setInterval(function(){
				...
				var wOffset = $body.width() - $content.width() + options.marginW*2
					,hOffset = $body.height() - $content.height() + options.marginH*2
				...
				if(h > windowHeight){
					if(h > 600) {
						// windowHeight = 600 - options.marginH*2;
					}
				...
				$body.animate(
					{
						/*'margin-left': -(w + wOffset)/2 + options.marginW
						,'margin-top': -(h + hOffset)/2 + options.marginH
						,*/'width': w + (parseInt($content.css("marginLeft"))*2)
					}
...
Number.isInteger = Number.isInteger || function(value) {
  ...
  $('document').ready(function () {
    if ($('.stage').length > 0) {
      $(window).scroll(function (event) {
          ...
          $('.stage .stage_image').css('transform', 'translate3d(0px, ' + (scroll/3) + 'px, 0px)');
      }).scroll();
    };
  });
  ...
  $('document').ready(function () {
        /*For not using fixed id's to target the container*/
      $('.accordion-header').on('click', function(e) {
          e.preventDefault();
          var $this = $(this);
          var $collapse = $this.closest('.accordion').find('.accordion-content');
  ...
  $(function() {
      ...
      $('a[href^="#"]:not([href="#"])').click(function(evt) {
        ...
        if($(this).hasClass('js-videoplay')){return;}
        if($(this).hasClass('js-modal')){return;}

        var off = 100;
        // if (isMobile()) {off=62};
          var target = $(this.hash);
          if ($('#tab-click-' + this.hash.substr(1)).length > 0) {
            target = $('#tab-click-' + this.hash.substr(1));
            $('#tab-click-' + this.hash.substr(1)).click();
          }
          // console.log("click scroll target"+target);
...
// image teaser click
  $(function() {
      $('.image-teaser .caption-layer.linked').on('click', function(evt) {
...
$(function() {
    ...
    $('.mh > [class*="col-"]').matchHeight();
    $('.content-block [class*="col-"] ').matchHeight();
    
    $('.text-image-teaser.type-vertical').closest('.row').addClass('row-eh row-flex-cols')

    $('[class*="col-"] .text-teaser--small ').matchHeight();
    $('.row-eh [class*="col-"] .multiteaser').matchHeight();

    $(' .text-image-teaser__hor [class*="col-"]:not(.nm)').matchHeight();

    $('.card .rowspace ').matchHeight();

    $('.slider_container .video-holder').matchHeight();

    $('.fullwidth [class*="col-"] .multiteaser').matchHeight();


      $('.mega-dropdown').each(function(index, el) {
          $(this).find('.dropdown-lvl-2 .lvl-2').matchHeight();
      });
});
...
$(document).ready(function(){
  ...
  $('div[class*="trans-y-"]').addClass("hidden-o").viewportChecker({
    classToAdd: 'animated a-fadeInUp',
    offset: 250,
  });
  $('div[class*="fade-up-headline"]').find(".headline").addClass("hidden-o").viewportChecker({
    classToAdd: 'animated a-fadeInUp',
    offset: 50,
  });
  $('div[class*="fade-headline"]').find(".headline").addClass("hidden-o").viewportChecker({
    classToAdd: 'animated a-fadeIn',
    offset: 50,
  });
  $('div[class*="fade-up-image"]').find("img").addClass("hidden-o").viewportChecker({
    classToAdd: 'animated a-fadeInUp',classToRemove: 'hidden-o',removeClassAfterAnimation: true,
    offset: 50,
  });
  $('div[class*="fade-image"]').find("img").addClass("hidden-o").viewportChecker({
    classToAdd: 'animated a-fadeIn',
    offset: 50,
  });
});
...
$(function() {
      $('.searchbar .js-open,.searchbar .js-close').on('click', function(e) {
          ...
          $('.navbar-header').toggleClass('left-out');
  ...
  $('.languageswitch select').selectric({maxHeight: 'auto'});

  $('#tx_indexedsearch').submit(function( event ) {
    $(this).attr('action', $(this).attr('action') + "&q=" + $('#tx-indexedsearch-searchbox-sword').val())
  });
...
$(function() {
  $('.powermail_form select').selectric();
});
...
***************************************************************************************************/
  // var lll = {
  //   'basketadd': 'Auf den Merkzettel',
  //   'basketremove': 'Vom Merkzettel entfernen',
  // }
  $( function(){


    var coo = Cookies.getJSON();
    var cooStore = coo['basket'];
    if (cooStore == undefined) {cooStore={}};

    for (var key in cooStore) {
       if (cooStore.hasOwnProperty(key)) {
          var obj = cooStore[key];
          $('.basket-add-to[data-id="'+key+'"]').addClass('pinned');
          $('.nav-icons').addClass('pinned');
          $('.merkzettel-meta').addClass('pinned');
          $('#menu-bottom').addClass('pinned');
          $('.basket-add-to[data-id="'+key+'"] .label').text(lll['basketremove']);
          $('.basket-add-to[data-id="'+key+'"]').attr('title',lll['basketremove']);
        }
    }

    $('.basket-add-to').click(function(event) {
        event.preventDefault();
        var toBePinned = $(this).data('id');

        if ($(this).hasClass('pinned')) {
            delete cooStore[toBePinned];
            if ($('body').hasClass('p-66')) {
                $('.basket-add-to[data-id="'+toBePinned+'"]').each(function(index, el) {
                  $(this).closest('.card').remove();
                  $(this).closest('.content-block').remove();
                });
                $('.card-listing').each(function(index, el) {
                  // if ($(this).find('.card').length == 0) {$(this).remove();};
                  if ($(this).find('.card').length == 0 && $(this).find('.content-block').length == 0) {$(this).remove();};
                });
                if ($('.card-listing').length == 0) {
                  $('.teaser-emptymessage').removeClass('hidden').addClass('hidden-o a-fadeInUp animated');
                  $('.teaser-download').remove();
                };
            ...
            $(this).attr('title',lll['basketadd']);
            $(this).find('.label').text(lll['basketadd']);
        } else {
            cooStore[toBePinned] = 1;
            $(this).attr('title',lll['basketremove']);
            $(this).find('.label').text(lll['basketremove']);
            $(this).addClass('pinned');
        }
        $('.nav-icons').removeClass('pinned');
        $('.merkzettel-meta').removeClass('pinned');
        if (Object.keys(cooStore).length > 0) {
          setTimeout(function(){ 
            $('.nav-icons').addClass('pinned'); 
            $('.merkzettel-meta').addClass('pinned'); 
          }, 300);
        ...
        // Cookies.set('basket', cooStore);
        Cookies.set('basket', cooStore, { expires: 365 });
    });
  ...
  // imageLightbox
    //  WITH DIRECTION REFERENCE
      ...
      closeButtonOn = function( instance ){
        $('<a href="#" id="imagelightbox-close">X</a>').appendTo('body').on('click touchend', function(){ $(this).remove(); instance.quitImageLightbox(); return false; });
      },
      ...
      {
        var $arrows = $( '<button type="button" class="btn imagelightbox-arrow imagelightbox-arrow-left"></button><button type="button" class="btn imagelightbox-arrow imagelightbox-arrow-right"></button>' );
    ...
    var selclass = 'a.imagelightbox';
    var insclass = $( selclass ).imageLightbox(
    {
    quitOnImgClick: true,                  // bool;            quit when the viewed image is clicked
    quitOnDocClick: true,                   // bool;            quit when anything but the viewed image is clicked
      onStart:   function() { overlayOn();closeButtonOn( insclass ); },
    ...
    // $('a.imagelightbox').each(function(index, el) {
    //   $(this.selector).imageLightbox();
    // });
...
/* image mouseover in card-listing
================================================== */
  $(function() {

      $(".card-listing img").unveil();

      $('.card-listing .card-asset img[data-imgover]')
      .mouseover(function() {
        var src = $(this).data('imgover');
        $(this).parent().append( "<div class='img-over pos-t0' style='position:absolute;pointer-events:none;'><img src='"+src+"' /></div>" );
      });
      $('.card-listing .card-asset').mouseout(function() {
        $(this).find( ".img-over" ).remove();
      });

  });
/* image mouseover in kitchendetail
================================================== */
  $(function() {


      $('div:not(.card-asset) img[data-imgover]')
      .mouseover(function() {
        var src = $(this).data('imgover');
        $(this).parent().find( ".img-over" ).remove();
        $(this).parent().prepend( "<div class='img-over pos-t0 col-sm-12' style='position:absolute;pointer-events:none;left:0;'><img src='"+src+"' /></div>" );
        $(this).parent().find('.img-over').hide().fadeIn();
      });
      $('div:not(.card-asset) ').mouseout(function() {
...
================================================== */
 ...
 $(function() {
      $('.str-slick-vertical .slick-slider-for').each(function(key, item) {

          var sliderIdName = 'slick-slider' + key;
          var sliderNavIdName = 'slick-sliderNav' + key;

          this.id = sliderIdName;
          $('.slick-slider-nav')[key].id = sliderNavIdName;
      ...
      $('.str-slick-horizontal').each(function (index, sliderWrap) {
        var $slider = $(sliderWrap).find('.slick-slider');
        var $sliderActions = $(sliderWrap).find('.actionholder');
        var centerMode = true;
        if ($(sliderWrap).closest('.modal').length>0) {
          centerMode = false;
        }
        ...
        // console.log(centerMode);
          ...
          $slider.slick({
                ...
                // ,appendArrows: $('.str-slick-horizontal .actionholder')
                // ,appendDots: $('.str-slick-horizontal .actionholder')
                ,appendArrows: $sliderActions
                ,appendDots: $sliderActions
          });
      });

      $('.str-slick-horizontal .slick-list').addClass('container');
      $('.str-slick-horizontal .slick-dots').addClass('container');
      // $('.str-slick-horizontal .text-box').matchHeight();
      ...
      // old v1
      // $('.str-slick-horizontal .slick-dots,.str-slick-horizontal .slick-slider > button').wrapAll('<div class="slick-shrink contentshrink" />');

      $('.str-slick-full .slick-slider').slick({
            slidesToShow: 1,
            slidesToScroll: 1,
            dots: true,
            arrows: false,
      ...
      $('.str-slick-show3 .slick-slider').slick({
            slidesToShow: 3,
            slidesToScroll: 1,
            responsive: [
                {
      ...
      /* slick slider logos
        ================================================== */
        $('.slick-footer-logos:not(.type-3)').slick({
          dots: false,
          infinite: true,
          arrows: true,
          speed: 300,
        ...
        $('.slick-footer-logos.type-3').slick({
          dots: false,
          infinite: true,
          arrows: true,
          speed: 300,
          ...
          responsive: [
            ...
            $('.slick-footer-logos .slick-slide').matchHeight();

        $('.str-slick-multiteaser .slick-slider').slick({
          slidesToShow: 1,
          slidesToScroll: 1,
          dots: true,
          arrows: false,
...
$(function() {
  ...
  $filterLogic = function () {
    var filterAll = {};
    var filterAllPlain = $('#filterForm option:not(:selected)').map(function() { 
        if ($(this).val()) {
          return $(this).parent().attr('name') + '-' +$(this).val();
        }
      }).get();
      for (var i = 0, len = filterAllPlain.length; i < len; i++) {
        filterAll[filterAllPlain[i]] = 1;
      }
    var classes = {};
    var foundItems = 0;
    // $('.card.a-fadeInUp').each(function(index, el) {
    $('.card.found').each(function(index, el) {
        foundItems++;
        // console.log(foundItems);
        var c = $(this).attr('class').toString().split(' ');
        for (var i = 0, len = c.length; i < len; i++) {
          if (!filterAll[c[i]]) {continue;};
          classes[c[i]] = 1;
        }
    });
    $('.filter-count').text('('+foundItems+')');
        // console.log(classes);
      $('#filterForm select option').prop('disabled', false);

      $('#filterForm select, .filter-holder').removeClass('sel');
      $('#filterForm select').each(function(index, el) {
          if( sel = $(this).prop('selectedIndex')){
            // console.log($(this).attr('name') +': '+sel);
            $(this).addClass('sel');
            $('.filter-holder').addClass('sel');
          }
      });

      for (var i = 0, len = filterAllPlain.length; i < len; i++) {
        if (classes[filterAllPlain[i]] == 1) {continue};
        var sp = filterAllPlain[i].split('-');
        $('#filterForm select[name="'+sp[0]+'"] option[value="'+sp[1]+'"]').prop('disabled', true);
      }
    $('#filterForm select').selectric('refresh');
...
================================================== */
$(function() {
  // $('.card-listing .card').addClass('hidden-o animated a-fadeInUp');

  // $filterLogic();
  $slideCards = function ($cards){
    $cards.addClass('hidden-o animated a-fadeInUp').show();
    $(window).trigger("lookup.unveil");
    // $cards.each(function(index, el) {
    //   // $(this).addClass('hidden-o animated a-fadeInUp').show();
    //   // $(this).addClass('a-fadeInUp');

    //   $(this).delay(index * 100).queue(
    //     function() {
    //       $(this).addClass('hidden-o animated a-fadeInUp').show();
  ...
  $('#filterForm').change(function(event) {
    $('.card-listing .card').removeClass('a-fadeInUp');
    $('.card-listing .card').removeClass('found').hide();
    ...
    for (var i = 0, len = parts.length; i < len; i++) {
      ...
      p = "." + pp[0] + '-' + pp[1];
      // alle, oder
      // $('.card-listing .card'+p).addClass('a-fadeInUp').show();
      pCollect += p;
    }
      ...
      if (count == 0) {
        // $('.card-listing .card').addClass('a-fadeInUp').show();
        $('.card-listing .card').addClass('found');
        $slideCards($('.card-listing .card'));
      } else {
        // alle, und
        // $('.card-listing .card' + pCollect).addClass('a-fadeInUp').show();
        $('.card-listing .card' + pCollect).addClass('found');
        $slideCards($('.card-listing .card.found' + pCollect));
  ...
  $(".kfilter.fronts select[name='kitchenColor'] option[value='5']").remove();

  $('#filterForm select').selectric();
  $('#filterForm').change();
});


/* filterFormJobs change
================================================== */
$(function() {
  ...
  // $('.filter-reset').click(function(event) {
  //   event.preventDefault();
  //   $('#filterFormJobs select').prop('selectedIndex',0);
  //   $('#filterFormJobs').change();
  // });

  $('#filterFormJobs').change(function(event) {
    $('.jobs-list-entry .row').hide();
    // $('select[name="remit"]').prop('disabled', true);

      if ($('select[name="typ"]').val() == 'Direkteinstieg') {
          $('select[name="remit"]').prop('disabled', false);
      } else {
        $('select[name="remit"] option[value="all"]').prop('selected', true);
        $('select[name="remit"]').prop('disabled', true);

      }

    var f = $('#filterFormJobs').serialize();
    var parts = f.split('&');
    var p = "";
    var pCollect = "";
    var count = 0;
    ...
    for (var i = 0, len = parts.length; i < len; i++) {
      ...
      // console.log(pp);
      if (!pp[1]) {continue;};
      if (pp[1] == 'all') {continue;};
      if (pp[1] == 0) {continue;};
      p = '[data-filter-' + pp[0] + '="' + decodeURIComponent(pp[1].replace(/\+/g, '%20')) + '"]';
      count++;
      pCollect += p;
    }

      if (count == 0) {
        $('.jobs-list-entry .row').show();
      } else {
        $('.jobs-list-entry .row' + pCollect).fadeIn();

      };
      $('#filterFormJobs select').selectric('refresh');
      // $(window).trigger("resize");
  }).change();

  if (window.location.hash != "") {
    $('select[name="typ"] option[value="'+window.location.hash.substr(1)+'"]').prop('selected', true);
    $('#filterFormJobs').change();
    
  };

  // $('#filterFormJobs select').selectric();
  // $('#filterFormJobs').change();
});
...
$(function() {
  ...
  $(".powermail_form_1 #powermail_field_checkbox1_2").change(function(event) {
      $('#powermail_check_question').remove();
      if ($(this).is( ":checked" )) {
        $('<input id="powermail_check_question" type="hidden" name="tx_powermail_pi1[field][check_question]" value="1" />').appendTo($(this).parent());
      }
  }).change();
...
$(function() {
  ...
  $('a[href$=".mp4"]').attr('data-lity',1);

  $(document).on('lity:close', function() {
    $vid = $('.lity-content video')[0];
    if ($vid) {$vid.pause()};
  });
...
$(function() {
  ...
  videocontainerActiveId = "";
  $(document).on('click', '.js-videoplay:not(.UC-yt-blocked)', function(event) {
    event.preventDefault();
    if (videocontainerActiveId != '') {
      var $activeContainer = $('#' + videocontainerActiveId).closest('.video-holder');
      $('#' + videocontainerActiveId).hide();
      $activeContainer.find('.embed-responsive').empty();
      $activeContainer.find('.js-videoplay').show();
      
    }

    var yt = $(this).data('youtubeid');
    var ytFrame = '<iframe width="1280" height="720" frameborder="0" src="https://www.youtube-nocookie.com/embed/'+yt+'?autoplay=1&showinfo=0&rel=0" webkitallowfullscreen mozallowfullscreen allowfullscreen=""></iframe>';
    var vh = $(this).closest('.video-holder');
    var vc = vh.find('.video-container');

    videocontainerActiveId = vc.attr('id');

    vc.find('.embed-responsive').empty().append(ytFrame);

    if (vh.hasClass('video-play-inline')) {
      $(this).hide();
      vc.show();
    } else {
      // vc.addClass('box');
  ...
  $(document).on('lity:close', function(event, instance) {
    if (videocontainerActiveId != '') {
      $('#' + videocontainerActiveId + ' .embed-responsive').empty();
      videocontainerActiveId = '';
    }
  });
...
================================================== */
    $(function(){
      // $(document).on('mousedown', '.js-img-viewbox.v2', function(event) {
      $('.js-img-viewbox').mousedown(function(event) {
        if ($(event.target).parent().hasClass('v2')) {
          $('body').addClass('viewbox-v2');
        } else {
          $('body').removeClass('viewbox-v2');
        }
      });
      $('.js-img-viewbox').viewbox({
        template: '<div class="viewbox-container"><div class="viewbox-body"><div class="viewbox-footer"></div><div class="viewbox-content"></div><div class="viewbox-header"></div></div></div>',
        setTitle: true,
        marginH: 100,
        marginW: 125,
        resizeDuration: 300,
...
$(function(){
  $( window ).resize(function() {
    $('.js-btn-samewidth p .ghost-button').css('width', 'auto');
    if ($(window).width() < 768) {
      return;
    }
    $('.js-btn-samewidth').each(function(index, el) {
      $(this).find('p .ghost-button').widest();
    });
...
window.addEventListener("consent_status", function (e) {
    ...
    // console.log(`consent_status: ${JSON.stringify(e.detail, null, 2)}`);
    if (!e.detail['YouTube Video']) {
      $(".yt-video, .yt-video .js-videoplay").addClass('UC-yt-blocked');
    } else {
      $(".yt-video, .yt-video .js-videoplay").removeClass('UC-yt-blocked');
    }
});
...
$(function (){
  $('.js-tab-accordion').on('click', function(e) {
    ...
    $collapse.collapse('toggle').toggleClass('active');
    var $tabs = $this.closest('.tabs');
    $tabs.find('.nav-tabs li').removeClass('active');
    $tabs.find('.nav-tabs [href="' + $this.data('target') + '"]').parent().addClass('active');
});

  $(document).on('shown.bs.tab', '.tabs .nav-tabs a[data-toggle="tab"]', function (e) {
    $(this).closest('.nav-tabs').find('li').removeClass('active');
    $(this).closest('li').addClass('active');
    $($(e.target).attr('href')).css('height','');
    $($(e.target).attr('href')).addClass('collapse in');

    $(this).closest('.tabs').find('.accordion-header').removeClass('collapsed');
    $($(e.target).attr('href') + '-acc .accordion-header').addClass('collapsed');
    

    $(window).trigger('resize');
    if ($('.str-slick-vertical .slick-slider').length > 0) {
      $('.str-slick-vertical .slick-slider').slick('resize');
    };

  });

  $('.tabs').each(function() {
    // console.log('tabs each');
    $(this).find('.nav-tabs > li:first-child a').tab('show');
  });  

  if (window.location.hash != "") {
    var th = window.location.hash.substr(1);
    if (th.indexOf("&") < 0 && $('#tab-click-' + th).length > 0) {
      $('#tab-click-' + th).click();
      $(window).scrollTop(0,0);
      $('html,body').animate({
        scrollTop: $('#tab-click-' + th).offset().top - 100
      }, 500, function() {});
    }
  };
    ...
    function trackEventDatalayer(category, action, label){
      console.log('trackEventDatalayer: '+category+','+action+','+label);
      if (window.location.hostname.indexOf("devcloud")  > -1) {
        console.log('trackEventDatalayer: no track: devcloud');
        return;
      }
      (window.dataLayer = window.dataLayer || []).push({
          "event": "datalayer-track",
...
$(document).ready(function () {
  ...
  $('.video-holder:not(.yt-video) .video-container + a').click(function(event) {
    event.preventDefault();
    var h = $(this).closest('.video-holder').find('.h3 .headline').text().trim();
    if (!h || h == '') {
      h = $(this).attr('title');
    }
    // console.log("video: " + h);
    trackEventDatalayer("Videos", h, window.location.pathname);
  });

  $('.video-holder.yt-video .video-container + a').click(function(event) {
    event.preventDefault();
    var h = $(this).attr('title') + '(' + $(this).data('youtubeid') + ')';
    // console.log("Youtube Video: " + h);
    trackEventDatalayer("Youtube Videos", h, window.location.pathname);
  });
...
function etrackerSetCookieConsent(e) {
  ...
  if(data && (data.event === 'consent_changed' || data.event === 'consent_status') && typeof _etracker !== 'undefined') {
    if(data['etracker'] === true) {_etracker.enableCookies(window.location.host);} else if (data['etracker'] === false){_etracker.disableCookies(window.location.host);}
  }
}
...
/* lang */
  ...
  $('document').ready(function () {
    var selected = $('select[name=switch]');
    $(selected).on('change', function() {
        ...
        if(url) {
              ...
              // document.cookie = "LANGDEF=" + url.replace(/\//g,'') + ";" + expires + ";path=/";
              document.cookie = "LANGDEF=" + urlParts[1] + ";" + expires + ";path=/";
            window.location = url;
        }
        return false;
    });
